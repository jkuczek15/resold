 ...
 */
class AdminQueryform extends \Magento\Framework\View\Element\Template
{
    /**
     * @var array
     */
    ...
    public function __construct(
        ...
        $collection = $this->_messagingFactory->create()->getCollection()->addFieldToFilter('role', 'admin')->addFieldToFilter('receiver_email', $sender_email)->addFieldToFilter('sender_id', 0)->addFieldToFilter('send_to', 'customer')->setOrder('chat_id', 'desc');
    ...
    /** get messages sent from admin **/
    public function getAdminInboxcollection()
    {
        $customerData = $this->customerSession->getCustomer();
        $sender_email=$customerData->getEmail();
        $collection = $this->_messagingFactory->create()->getCollection()->addFieldToFilter('role', 'admin')->addFieldToFilter('receiver_email', $sender_email)->addFieldToFilter('send_to', 'customer')->setOrder('chat_id', 'desc')->getData();
        return $collection;
    }
