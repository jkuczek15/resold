...
namespace Ced\CsMessaging\Block\Adminhtml\Vendor\MassMessage\Edit\Tab;
...
class Main extends Generic implements TabInterface
{
    ...
    {
        $customerIds = $this->_registry->registry('vendor_ids');
        $isElementDisabled = false;
        /** @var \Magento\Framework\Data\Form $form */
        $form = $this->_formFactory->create();
        $formValues = ['vendor_ids'=>implode($customerIds, ','),'mass_messaging'=>1];
        $fieldset = $form->addFieldset('base_fieldset', ['legend' => __('Compose Message')]);
        $fieldset->addField('vendor_ids', 'hidden', ['name' => 'vendor_ids']);
        $fieldset->addField('mass_messaging', 'hidden', ['name' => 'mass_messaging']);
