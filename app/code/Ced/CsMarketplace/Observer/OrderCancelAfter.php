...
Class OrderCancelAfter implements ObserverInterface
{
    ...
    { 
        ...
        $helper->logProcessedData($order->getIncrementId(), \Ced\CsMarketplace\Helper\Data::SALES_ORDER_CANCELED);
        try {
            ...
            if (count($vorders) > 0) {
                foreach ($vorders as $vorder) {
                    ...
                    $helper->logProcessedData($vorder->getData(), \Ced\CsMarketplace\Helper\Data::VORDER_CANCELED);
                    $this->_objectManager->create('Ced\CsMarketplace\Helper\Mail')->sendOrderEmail($order, \Ced\CsMarketplace\Model\Vorders::ORDER_CANCEL_STATUS,$vorder->getVendorId(),$vorder);
