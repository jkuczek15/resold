...
class Js
{
    ...
    protected function insertRequireJsFiles(&$html)
    {
        ...
        foreach ($tagList as $tag) {
            ...
            $html = HtmlParser::replaceIntoHtml($html, '', $tag->getStart(), $tag->getEnd());
            break;
        }
        if (null === $key) {
            return $this;
     ...
     * @return string
     */
    protected function getRequireJsResultFilePath($key)
    {
    ...
    {
        return $this->dataHelper->getRootCacheUrl()
    ...
    protected function getRequireJsResultFileName($key)
    {
        ...
        sort($urlList);
        return md5(
            $key . '||' . $this->fileHelper->getLastFileChangeTimestampForUrlList($urlList)
        ). '.js';
    }
}
