...
class ListProduct extends AbstractProduct implements IdentityInterface
{
    ...
    protected function _getProductCollection()
    {
        ...
        if(isset($_GET['local_global']) && ($_GET['local_global'] == 227 || $_GET['local_global'] == 224))
        {
          ...
          $longitude_expr = 'IF(at_longitude.value > 0, at_longitude.value, at_longitude_default.value)';
          $haversine_expr = 'SQRT(69.1*69.1*('.$latitude_expr.' - '.$input_lat.')*('.$latitude_expr.' - '.$input_lat.') + 53*53*('.$longitude_expr.' - '.$input_long.')*('.$longitude_expr.' - '.$input_long.'))';

          // add custom select attributes
          // implementation of haversine formula to caclulate great circle distance
          $this->_productCollection->getSelect()->columns(
          array(
            'distance_miles' => new \Zend_Db_Expr($haversine_expr)
          ))->where($haversine_expr . ' < '.$distance);

          // add custom filters to product collection
          ...
          // $this->_productCollection->getSelect()->assemble();
          // $this->_productCollection->getSelect()->__toString();
